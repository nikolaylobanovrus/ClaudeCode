...
// Прогрессивная шкала НДФЛ (ст. 224 НК). Плоские 13% — упрощение, которое
// врёт сразу в две стороны: у человека с доходом выше порога возврат по вычету
// идёт по ставке ВЕРХНЕЙ ступени (вычет снимает доход именно оттуда), а налог
// с продажи дорогой недвижимости занижается, и это уже недоимка со штрафом.
//
//   2021–2024: 13% до 5 млн, 15% свыше (ФЗ от 23.11.2020 № 372-ФЗ).
//   с 2025:    основная база — 13 / 15 / 18 / 20 / 22% на порогах
//              2,4 / 5 / 20 / 50 млн (ФЗ от 12.07.2024 № 176-ФЗ);
//              доходы от продажи имущества — отдельная шкала по п. 1.1
//              ст. 224: 13% до 2,4 млн и 15% свыше, выше не растёт.
const SCALE_MAIN = {
  2022: [[5_000_000, 0.13], [Infinity, 0.15]],
  2023: [[5_000_000, 0.13], [Infinity, 0.15]],
  2024: [[5_000_000, 0.13], [Infinity, 0.15]],
  2025: [[2_400_000, 0.13], [5_000_000, 0.15], [20_000_000, 0.18],
         [50_000_000, 0.20], [Infinity, 0.22]],
};
const SCALE_SALE = {
  2022: [[5_000_000, 0.13], [Infinity, 0.15]],
  2023: [[5_000_000, 0.13], [Infinity, 0.15]],
  2024: [[5_000_000, 0.13], [Infinity, 0.15]],
  2025: [[2_400_000, 0.13], [Infinity, 0.15]],
};

// Налог с базы по шкале года. kind: "main" — основная база (зарплата),
// "sale" — доходы от продажи имущества (с 2025 своя, более пологая шкала).
export function taxOn(base, year, kind = "main") {
  ...
  for (const [upTo, rate] of table) {
    if (b <= prev) break;
    tax += (Math.min(b, upTo) - prev) * rate;
    prev = upTo;
  }
  return Math.round(tax);
}

// Ставка на «верхнем рубле» базы — по ней возвращается вычет: он снимает доход
// с самой верхней ступени, а не с первой.
export function marginalRate(base, year, kind = "main") {
  const table = (kind === "sale" ? SCALE_SALE : SCALE_MAIN)[Number(year)] ||
                (kind === "sale" ? SCALE_SALE : SCALE_MAIN)[2025];
  const b = Math.max(0, Number(base) || 0);
  let prev = 0;
  for (const [upTo, rate] of table) {
    if (b <= upTo) return rate;
    prev = upTo;
  }
...
export const refundDeadlineYear = (year) => Number(year) + 3;
...
// Когда строка 030 ОБЯЗАНА присутствовать, а когда запрещена. Правило жёсткое
// и проверяется контрольным соотношением прямо в схеме ФНС:
//   2022, 2023, 2025 — при коде объекта 1 (жилой дом) или 7;
//   2024            — только при коде 1.
// Ловушка в том, что «жилой дом с земельным участком» в форме 2024 получает
// код 6, а не 1: наш код заполнял способ приобретения по виду объекта, файл
// уходил с кодом 6 И способом приобретения, и ЛК ФНС его отвергал. Проверять
// надо ИМЕННО код в конкретной форме, а не смысл («дом это или не дом»).
export function propertyNeedsBuildCode(objectCode, year) {
  const allowed = Number(year) === 2024 ? ["1"] : ["1", "7"];
  return allowed.includes(String(objectCode));
}
