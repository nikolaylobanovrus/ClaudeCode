...
//
// За 2025 год декларация печатается на ПОДЛИННОМ бланке ФНС (приказ от
// 20.10.2025 № ЕД-7-11/913@): страницы официального PDF со штрихкодами и
// знакоместами + значения в клетках (см. blank2025.js) — вид совпадает с
// выгрузкой из ПО ФНС. За 2022–2024 годы — человекочитаемая реплика без
// машиночитаемых штрихкодов (дисклеймер в подвале листа); эти годы
// подаются XML-файлом через ЛК ФНС, PDF служит копией для клиента.
...
export async function buildDeclarationPdf(model) {
  // Актуальная форма (2025+) — печать на официальном бланке ФНС.
  if (model.year >= 2025) return buildOfficialPdf2025(model);
