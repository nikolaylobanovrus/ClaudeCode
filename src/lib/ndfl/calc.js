...
export function computeDeclaration(draft) {
  ...
  // Лист «Расчёт к Приложению 5» есть в бланке 2025 года и обязателен там, где
  // заявлены взносы по страхованию жизни (из него берётся строка 160 — Порядок,
  // п. 78) или взносы на ИИС (раздел 2 листа — п. 106). В наших бланках за
  // 2022–2024 этого листа нет: при нарезке официальных форм его не взяли, а
  // перевыпустить ассет пока не из чего. Молчать об этом нельзя — человек
  // отнесёт в инспекцию неполный комплект и узнает об этом от инспектора.
  if (
    Number(draft.year) <= 2024 &&
    (num(draft.insurance?.amount) > 0 || num(draft.iis?.contribution) > 0)
  ) {
    warnings.push(
      "За этот год в печатном комплекте не хватает листа «Расчёт к Приложению 5» — " +
        "его требуют, когда заявлены взносы по страхованию жизни или на ИИС. " +
        "Файл для личного кабинета (XML) заполнен полностью, поэтому надёжнее подать " +
        "декларацию через личный кабинет ФНС. Если подаёте на бумаге — напишите нам, " +
        "пришлём этот лист заполненным."
    );
  }
