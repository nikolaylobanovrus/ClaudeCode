...
const MAP_22_23 = {
  ...
  app6: { x: 422.7, y010: 673.2, y020: 647.7, y050: 551.0, y060: 525.5, y070: 489.0, y080: 463.5, y160: 52.5 },
...
const MAPS = {
  ...
  2024: {
    ...
    // Раздел 6 «Расчёт налоговых вычетов на долгосрочные сбережения граждан»
    // появился в форме 757@ и состоит из ТРЁХ строк, а не семи, как в бланке
    // 2025 года: 240 — вычет по пп. 2 п. 1 ст. 219.2 (договор ПДС), 250 — по
    // пп. 3 (ИИС, открытый с 2024), 260 — предоставленный в упрощённом
    // порядке. Ровно эти три величины требует и XSD ФНС за 2024 год.
    // Координаты сняты из текстового слоя бланка (pdftotext -bbox) и сверены
    // по семи уже работающим строкам того же листа.
    app5b: { 140: 716, 160: 550, 171: 440, 180: 411, 190: 317, 200: 288, 210: 242,
             240: 130.0, 250: 101.6, 260: 73.1 },
    ...
    app6: { x: 422.7, y010: 678.4, y020: 652.9, y050: 556.2, y060: 530.7, y070: 494.2, y080: 468.7, y160: 64.9 },
...
export async function buildOfficialPdfLegacy(model) {
  ...
  function fillApp1Sale(pen, items) {
    ...
    items.forEach((o, i) => {
      ...
      // 070 — доход, ПРИНИМАЕМЫЙ для налогообложения, а не цена из договора.
      // Когда цена ниже 70% кадастровой стоимости, доходом считается кадастр ×
      // 0,7 (ст. 214.10 НК), и именно эта величина идёт в строку 070 — она же
      // строка 060 «Расчёта к Приложению 1». Печаталась цена по договору, и
      // Приложение 1 расходилось с Разделом 2, где доход уже кадастровый.
      // Контрольное соотношение ФНС (Раздел 2 стр. 010 = сумма строк 070
      // Приложения 1) не сходилось — инспекция запросила бы пояснения.
      pen.money(o.taxable, A.innX, A.sumY[i], 13); // 070 сумма дохода
  ...
  function fillApp6(pen) {
    ...
    const d = sale.ded;
    if (d.home.standard > 0) pen.money(d.home.standard, A.x, A.y010, 8); // 010 вычет 1 млн
    if (d.home.expenses > 0) pen.money(d.home.expenses, A.x, A.y020, 8); // 020 расходы
    // Пункт 2 бланка — иное недвижимое (гараж, машиноместо, апартаменты).
    if (d.realtyOther.standard > 0) pen.money(d.realtyOther.standard, A.x, A.y050, 8); // 050 вычет 250 тыс
    if (d.realtyOther.expenses > 0) pen.money(d.realtyOther.expenses, A.x, A.y060, 8); // 060 расходы
    if (d.movable.standard > 0) pen.money(d.movable.standard, A.x, A.y070, 8); // 070 вычет 250 тыс
    if (d.movable.expenses > 0) pen.money(d.movable.expenses, A.x, A.y080, 8); // 080 расходы
  ...
  function fillApp5b(pen) {
    ...
    // Раздел 6: вычет на долгосрочные сбережения (ст. 219.2). Есть только в
    // форме 2024 года — в 2022/2023 статьи ещё не существовало.
    //
    // Расчёт считал этот вычет и за 2024 год, XML его выгружал, а на бумаге
    // его не было вовсе: человек с договором ПДС на 80 000 ₽ получал комплект,
    // в котором про эти 80 000 не сказано ни строчки. Печать и выгрузка
    // говорили разное об одних и тех же деньгах.
    const sv = calc.savings;
    if (A[240] && sv?.declared > 0) {
      if (sv.pds > 0) pen.money(sv.pds, X, A[240], 12); // 240 ПДС (пп. 2)
      if (sv.iis3 > 0) pen.money(sv.iis3, X, A[250], 12); // 250 ИИС с 2024 (пп. 3)
      if (sv.simplified > 0) pen.money(sv.simplified, X, A[260], 12); // 260 упрощённый порядок
    }
  }
