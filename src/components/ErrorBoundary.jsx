...
// Страница уходит: вкладку закрывают, смахивают или уводят назад. Ошибки,
// случившиеся в этот момент, к качеству сайта отношения не имеют.
let leaving = false;
...
export default class ErrorBoundary extends Component {
  ...
  componentDidCatch(error, info) {
    // Человек уже уходит со страницы — это не авария, а оборванная загрузка.
    //
    // Нажал объявление, передумал, смахнул вкладку — браузер обрывает запрос
    // недогруженного куска, и ленивый импорт падает с той же ошибкой, что и
    // настоящая поломка. Отличить их по тексту нельзя, а по состоянию
    // страницы — можно: тот, кто ушёл, уже не видит ни ошибки, ни того, что
    // мы ему покажем. Такие случаи не считаем и не перезагружаем: иначе
    // цель render_error меряет отказы от рекламы, а не наши поломки.
    //
    // 11–13.09.2026 таких набралось пять, все с iPhone, четыре — новые
    // посетители с одним просмотром страницы. Я сперва решил, что это
    // выкладка отняла у них файлы, и ошибся: у нового посетителя нет старой
    // вкладки.
    if (isLeaving()) return;
