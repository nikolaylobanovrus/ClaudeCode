...
function WizardBody() {
  ...
  useEffect(() => {
    ...
    const saved = loadDraft();
    if (!saved) {
      // ?korr=1 из рекламы «ошиблись в декларации» ничего не делает: с 08.09
      // уточнённая выбирается переключателем на первом шаге, по умолчанию
      // «Первичная». Проставлять номер корректировки по ссылке нельзя — по
      // ней приходит и автотаргетинг с обычными запросами, а первичную
      // декларацию с номером корректировки ФНС не примет.
