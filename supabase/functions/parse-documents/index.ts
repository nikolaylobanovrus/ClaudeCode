...
//
// Держим 10, а не 20: двадцать страниц — это втрое больше токенов на вызов,
// и распознавание клиентского набора дорожает с ~$0,03 до ~$0,10. Решение
// владельца (25.08). Кто упёрся в лимит — загружает вторым заходом.
const MAX_PAGES = 10;
  ...
  properties: {
    ...
    incomes: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["name", "inn", "kpp", "oktmo", "income", "withheld", "monthly"],
        properties: {
          ...
          monthly: {
            type: "array",
            description:
              "Доход ПО МЕСЯЦАМ из раздела 3 справки о доходах: ровно 12 чисел, январь…декабрь, " +
              "без нарастающего итога — сумма ЗА месяц. " +
              "ГЛАВНОЕ: в разделе 3 за ОДИН месяц бывает НЕСКОЛЬКО строк с разными кодами дохода " +
              "(2000 — зарплата, 2002 и 2003 — премии, 2012 — отпускные, 2300 — больничный). " +
              "Все строки одного месяца НАДО СЛОЖИТЬ в одно число. Например, если за декабрь стоят " +
              "строки «12 2000 45455.00» и «12 2002 400000.00», то декабрь = 445455.00, а не 45455.00. " +
              "ПРОВЕРЬ СЕБЯ: сумма твоих 12 чисел обязана совпасть с общей суммой дохода из раздела 5. " +
              "Не совпала — ты потерял строки, перечитай раздел 3 целиком. " +
              "Месяц без выплат — 0. Если раздела 3 в документе нет, верни пустой массив. " +
              "Это нужно для вычета на детей: он даётся по тот месяц, в котором доход нарастающим " +
              "итогом ещё не превысил предел, и именно премия решает, сколько месяцев получится.",
            items: money,
          },
        },
      },
...
function systemPrompt(year: number, types: string[]) {
  ...
  return [
    ...
    "- Самопроверка помесячного дохода: сумма 12 чисел в monthly обязана совпасть с общей суммой дохода за год (раздел 5 справки). Чаще всего расходится потому, что за один месяц в разделе 3 несколько строк с разными кодами дохода (зарплата + премия + отпускные), а сложена только одна. Сложи все строки месяца.",
