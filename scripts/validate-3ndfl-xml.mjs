...
for (const year of [...YEARS].sort((a, b) => a - b)) {
  ...
  const model = buildDeclarationModel(sampleDraft(year));
  const { filename, bytes } = buildDeclarationXml(model);
  const xmlPath = join(tmp, filename);
  ...
  try {
    ...
    console.log(`✓ ${year}: OK — XML соответствует схеме ФНС`);
  } catch (e) {
    failed++;
    console.log(`✗ ${year}: XML НЕ прошёл схему:`);
