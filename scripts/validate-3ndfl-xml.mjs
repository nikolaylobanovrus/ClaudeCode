...
const scenarios = [
  ...
  // Декларация БЕЗ единого социального вычета: только дети. Лист «Приложение 5»
  // раньше в такой не попадал вовсе — Раздел 2 уменьшал базу, а подтверждения
  // не было ни на бумаге, ни в XML. Проверяем, что лист на месте.
  {
    tag: "только дети, без соцвычетов",
    patch: {},
    apply: (d) => { d.types = ["deti"]; },
  },
  // То же для долгосрочных сбережений: свой раздел на том же листе.
  {
    tag: "только сбережения, без соцвычетов",
    patch: {},
    minYear: 2025,
    apply: (d) => { d.types = ["sberezheniya"]; },
  },
  {
